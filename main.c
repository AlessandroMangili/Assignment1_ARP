...
int main() {
    ...
    if (file == NULL) {
        perror("[MAIN]: Error opening the JSON configuration file");
        return EXIT_FAILURE;
    }
    ...
    if (json == NULL) {
        perror("[MAIN]: Error parsing the JSON configuration file");
        return EXIT_FAILURE;
    }
    ...
    if (sync_sem == SEM_FAILED) {
        perror("[MAIN]: Failed to open the semaphore for the sync");
    ...
    char *inputs[N_PROCS - 1][16] = {
        {"./src/Binary/ServerSub", drone_write_size_fd_str, drone_write_key_fd_str, input_read_key_fd_str, drone_write_obstacles_fd_str, drone_write_targets_fd_str, pos_str, vel_str, force_str, map_x, map_y, NULL}, 
