...
Since the `Konsole` terminal does not support resizing via command, it is not possible to set a default size for the terminal window. As a result, you must either manually adjust its size or adapt the JSON file parameters to match the terminal's dimensions. A potential future improvement could involve replacing Konsole with a terminal that supports resizing or developing custom terminal solutions.
